...
# (Re-use your existing df_params, df_params_bare build is here for safety)
unit_to_bare = {
...
df_params_bare = df_params.rename(columns=unit_to_bare).copy()
df_params_bare['Algorithm'] = (
    df_params_bare['Algorithm']
      .replace({
          'BO': 'Bayesian',
...
champions_df = df_params_bare[df_params_bare['Algorithm'].isin(['Bayesian', 'CMA-ES'])].copy()
baseline_mask = df_params_bare['Algorithm'].str.contains('Baseline', case=False, na=False)
assert baseline_mask.any(), "Could not find a baseline row in champion_parameters_ranked.csv"
baseline_series = df_params_bare.loc[baseline_mask].iloc[0]
...
# Colors (reuse your palette if defined)
if 'PALETTE' not in globals():
    PALETTE = {'BO': '#4c72b0', 'CMA-ES': '#55a868', 'Baseline': '#c44e52'}
if 'ALGO_COLOR_MAP' not in globals():
    ALGO_COLOR_MAP = {'Bayesian': PALETTE['BO'], 'CMA-ES': PALETTE['CMA-ES']}

def plot_champion_strategy_strip(
    champions_df,
    baseline_series,
    param_cols,
    param_ranges,
    param_label=None,
    custom_ticks=None,
    outfile='plot_champion_strategy_strip.png'
):
    y_map = {'CMA-ES': 0, 'Bayesian': 1}
    y_labels = ['CMA-ES', 'Bayesian']

    # Wider right margin reserved for legend; taller for clarity
    fig, axes = plt.subplots(1, len(param_cols), figsize=(18, 6), sharey=True)
    if len(param_cols) == 1:
        axes = [axes]
    ...
    for idx, p in enumerate(param_cols):
        ...
        # Lock axes exactly to bounds (prevents the illusion of "out-of-bounds")
        xmin, xmax = param_ranges[p]
        ax.set_xlim(xmin, xmax)

        # Dotted bounds
        ax.axvline(xmin, color='k', linestyle=':', linewidth=1.2, alpha=0.9, zorder=1)
        ax.axvline(xmax, color='k', linestyle=':', linewidth=1.2, alpha=0.9, zorder=1)

        # Baseline
        base_x = baseline_series[p]
        ax.axvline(base_x, color=PALETTE['Baseline'], linestyle='--', linewidth=2, alpha=0.9, zorder=2)
        ax.scatter(base_x, 0.5, marker='X', s=140, color=PALETTE['Baseline'],
                   edgecolor='white', linewidth=1.2, zorder=6)

        # Champions per algorithm
        for alg in ['Bayesian', 'CMA-ES']:
            sub = champions_df[champions_df['Algorithm'] == alg]
            for _, row in sub.iterrows():
                x = row[p]
                y = y_map[alg] + float(rng.normal(0, 0.035))
                ax.scatter(
                    x, y,
                    marker='*', s=200,
                    color=ALGO_COLOR_MAP.get(alg, 'gray'),
                    edgecolor='white', linewidth=1.2,
                    alpha=0.95, zorder=7
                )
                run_lbl = str(row.get('Source Run', '')).replace('Run ', 'R')
                if run_lbl and run_lbl.lower() != 'baseline':
                    ax.text(
                        x, y + (0.10 if alg == 'Bayesian' else -0.10),
                        run_lbl,
                        ha='center', va='center',
                        fontsize=10, fontweight='bold', color=ALGO_COLOR_MAP.get(alg, 'gray'),
                        zorder=8,
                        path_effects=[pe.withStroke(linewidth=2, foreground='white')]
                    )

        # Ticks/labels
        if custom_ticks and p in custom_ticks:
            ax.set_xticks(custom_ticks[p])
        ax.set_xlabel(param_label[p] if param_label and p in param_label else p, fontsize=12)
        ...
        ax.set_title(param_label[p] if param_label and p in param_label else p, fontsize=13, pad=8)

    # Custom legend — placed OUTSIDE the plot area to avoid any overlap
    handles = [
        Line2D([0], [0], marker='*', color='w',
               markerfacecolor=ALGO_COLOR_MAP['Bayesian'], markeredgecolor='white',
               markersize=12, linewidth=0, label='BO Champion'),
        Line2D([0], [0], marker='*', color='w',
               markerfacecolor=ALGO_COLOR_MAP['CMA-ES'], markeredgecolor='white',
               markersize=12, linewidth=0, label='CMA-ES Champion'),
        Line2D([0], [0], marker='X', color=PALETTE['Baseline'],
               markerfacecolor=PALETTE['Baseline'], markeredgecolor='white',
               markersize=10, linewidth=2, linestyle='--', label='Manual Baseline'),
        Line2D([0], [0], color='k', linestyle=':', linewidth=1.2, label='Search Bounds')
    ]

    # Reserve right margin for legend & top margin for title
    fig.suptitle('Champion Parameter Strategy (per Algorithm)', y=0.98, fontsize=18, fontweight='bold')
    plt.tight_layout(rect=[0.0, 0.0, 0.82, 0.93])  # leave space on right & top

    fig.legend(
        handles=handles,
        title='Parameter Strategy',
        loc='upper left',
        bbox_to_anchor=(0.84, 0.98),   # outside, top-right margin
        frameon=True,
        fontsize=11,
        title_fontsize=12
    )
